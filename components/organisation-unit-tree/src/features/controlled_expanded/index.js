...
Given('the org unit tree is being provided with the required props to control the expanded state', () => {
    ...
    cy.get('[data-test="dhis2-uiwidgets-orgunittree-wrongpropsmessage"]').should('not.exist')
})
...
Then('the path should close', () => {
    cy.get('@providedPaths').then(providedPaths => {
        ...
        const hiddenChildrenIds = providedIds.reduce(
            (acc, cur) => {
...
Then('the org unit tree should display an error', () => {
    cy.get('[data-test="dhis2-uiwidgets-orgunittree-wrongpropsmessage"]').should(
        'contain',
