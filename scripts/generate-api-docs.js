...
const componentsDir = path.join(__dirname, '../collections/ui/src')
const uiParts = fs
    .readdirSync(componentsDir)
    .filter((pathSegment) => {
        const curPath = path.join(componentsDir, pathSegment)
        return fs.lstatSync(curPath).isDirectory()
    })

// const components = uiParts.filter(part => !['constants', 'icons'].includes(part))
...
//     const entries = fg.sync(`${component}/**/*.js`, {
//         ignore,
//     })
...
const icon = path.join(__dirname, '../collections/ui/src/icons')
console.info(`Generating API documentation for: ${path.basename(icon)}`)

const entries = fg.sync(`${icon}/**/*.js`, {
    ignore,
})
...
const pkg = require(path.join(__dirname, '../collections/ui/package.json'))
const markdown = asts
    .filter((c) => c.exported)
    .map((c) => {
        try {
