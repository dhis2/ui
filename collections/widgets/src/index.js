/*
 * For backwards compatibility we export the components that were
 * available in `ui-widgets` here.
 */
export { HeaderBar } from '@dhis2-ui/header-bar'
export { OrganisationUnitTree } from '@dhis2-ui/organisation-unit-tree'
export { CheckboxField } from '@dhis2-ui/checkbox'
export { FieldGroup } from '@dhis2-ui/field'
export { FileInputField } from '@dhis2-ui/file-input'
export { InputField } from '@dhis2-ui/input'
export { SingleSelectField, MultiSelectField } from '@dhis2-ui/select'
export { Pagination } from '@dhis2-ui/pagination'
export { SwitchField } from '@dhis2-ui/switch'
export { TextAreaField } from '@dhis2-ui/text-area'
export { Transfer, TransferOption } from '@dhis2-ui/transfer'
export {
    DataTable,
    DataTableBody,
    DataTableCell,
    DataTableColumnHeader,
    DataTableFoot,
    DataTableHead,
    DataTableRow,
    DataTableToolbar,
} from '@dhis2-ui/table'
// TODO export { SharingDialog } from '@dhis2-ui/sharing-dialog'
